...
def buy():
    ...
    if request.method == "POST":
        ...
        stock_totalprice = stock_price * shares

        if cash < stock_totalprice:
            return apology("There is no sufficient cash to buy the shares!")
        else:
            db.execute("UPDATE users SET cash = ? WHERE id = ?",cash - stock_totalprice, user_id)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if shares <= 0:
            return apology("Number of shares must be more than zero!")
        ...
        if shares_owned < shares:
            return apology("You don't have enough share to sell!")
